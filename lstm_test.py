...
class Config:
    ...
    # Model save paths (different from transformer and GCN)
    MODEL_SAVE_PATH_INNER = "best_inner_camera_lstm_model.pth"
    MODEL_SAVE_PATH_OUTER = "best_outer_camera_lstm_model.pth"

    # Results save paths (different from other models)
    RESULTS_CSV = "lstm_detailed_results.csv"
    HYPERPARAMS_CSV = "lstm_hyperparameters.csv"
    DATA_STATS_CSV = "lstm_data_statistics.csv"
    CONFUSION_MATRIX_DIR = "lstm_confusion_matrices"
...
idx_to_label = {i: label for i, label in enumerate(ACTION_LABELS)}
NUM_CLASSES = len(ACTION_LABELS)
INPUT_SIZE = 32 * 3  # 96 features per frame
...
class SkeletonActionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Improved normalization - pelvis-centered
        ...
        normalized_sequence = normalized_sequence.reshape(-1, INPUT_SIZE)
        ...
        if self.augment and random.random() < 0.3:
            ...
            if random.random() < 0.2:
                ...
                new_length = int(len(normalized_sequence) * scale_factor)
                if new_length > 10:  # Ensure minimum length
                    indices = np.linspace(0, len(normalized_sequence) - 1, new_length)
                    normalized_sequence = np.array(
                        [
                            np.interp(
                                indices,
                                np.arange(len(normalized_sequence)),
                                normalized_sequence[:, i],
                            )
        ...
        padded_sequence = np.zeros(
            (config.SEQUENCE_LENGTH, INPUT_SIZE), dtype=np.float32
        )
...
# --- 2. Enhanced Bidirectional LSTM Model ---
class ActionRecognitionLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # LSTM forward pass
        lstm_out, _ = self.lstm(x, (h0, c0))  # (batch, time, hidden*2)
...
def train_and_evaluate_model(
    ...
    model = ActionRecognitionLSTM(
        INPUT_SIZE, config.HIDDEN_SIZE, config.NUM_LAYERS, NUM_CLASSES, config.DROPOUT
    ).to(device)
    ...
    print(f"\n{'='*60}")
    print(f"Training Bidirectional LSTM Model for {view_name} View")
    print(f"{'='*60}")
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        # Print progress every 10 epochs or when new best is found
        if (epoch + 1) % 10 == 0 or val_accuracy > best_accuracy:
            print(
    ...
    cm_path = os.path.join(
        config.CONFUSION_MATRIX_DIR, f"lstm_{view_name.lower()}_confusion_matrix.png"
    )
    plot_confusion_matrix(
        ...
        ACTION_LABELS,
        cm_path,
        f"Bidirectional LSTM {view_name} View - Confusion Matrix",
    )
...
def create_splits_by_view(
    sequences_dict, view_name, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1
):
    """Create stratified splits for a view"""
    print(f"\n--- Creating Stratified Split for {view_name} View ---")

    sequence_to_label = {}
    for seq_id, frames in sequences_dict.items():
        if frames:
            sequence_to_label[seq_id] = frames[0].get("action_label")
    ...
    for label, sequences in label_to_sequences.items():
        ...
        if n_train <= 0:
            n_train = 1
            n_test = max(1, int((n_total - 1) / 2))
            n_val = n_total - n_train - n_test

        train_seq_ids.extend(sequences[:n_train])
        val_seq_ids.extend(sequences[n_train : n_train + n_val])
        test_seq_ids.extend(sequences[n_train + n_val :])
    ...
    output_path = f"lstm_split_config_{view_name.lower()}.json"
    with open(output_path, "w") as f:
        json.dump(split_config, f, indent=4)
    print(f"Saved {view_name} split to {output_path}")
    return split_config
...
if __name__ == "__main__":
    print("=" * 80)
    print(" " * 25 + "BIDIRECTIONAL LSTM ACTION RECOGNITION EXPERIMENT")
    print("=" * 80)
    ...
    # Create splits for each view
    inner_split_config = create_splits_by_view(inner_sequences, "Inner")
    outer_split_config = create_splits_by_view(outer_sequences, "Outer")

    all_results = []
    all_data_stats = []

    # Process for each camera view
    for view_name, sequences_dict, save_path, split_config in [
        ("Inner", inner_sequences, config.MODEL_SAVE_PATH_INNER, inner_split_config),
        ("Outer", outer_sequences, config.MODEL_SAVE_PATH_OUTER, outer_split_config),
    ]:
        print(f"\n{'='*60}")
        print(f"Processing {view_name} View")
        print(f"{'='*60}")

        view_train_ids = split_config["train_sequences"]
        view_val_ids = split_config["validation_sequences"]
        view_test_ids = split_config["test_sequences"]
    ...
    if all_results:
        ...
        stats_df.to_csv(config.DATA_STATS_CSV, index=False)

        # Print final summary table
        print("\n" + "=" * 100)
        print(" " * 30 + "BIDIRECTIONAL LSTM FINAL RESULTS SUMMARY")
        print("=" * 100)
        ...
        print(f"Hyperparameters saved to: {config.HYPERPARAMS_CSV}")
        print(f"Data statistics saved to: {config.DATA_STATS_CSV}")
