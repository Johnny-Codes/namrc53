...
class MultimodalTrainer:
    ...
    def __init__(
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            ...
            T_mult=1,
            eta_min=5e-7,  # Never go below this
            last_epoch=-1,
        )
        ...
        #     gamma=0.8  # Gentler decay
        # )

        print(f"🔧 Using improved learning rate scheduler with min LR protection")

        # Disable mixed precision for maximum stability
        self.use_amp = False
        print("🔍 Running in FULL PRECISION mode for maximum stability")
        ...
        # Fix: Convert class_counts list to dictionary format expected by WeightedLossCalculator
        ...
        self.class_weights = weight_calculator.calculate_class_weights(
            class_counts_dict, num_classes, device  # Pass dict instead of list
        )

        # 2. MAXIMUM class weight capping
        max_weight = 3.0  # Even more aggressive - down from 5.0
        self.class_weights = torch.clamp(self.class_weights, max=max_weight)
        print(f"⚖️ Class weights clipped to MAXIMUM value of {max_weight}")

        self.criterion = nn.CrossEntropyLoss(weight=self.class_weights)

        # Fix: Pass class_counts_dict instead of class_counts list to print_weight_analysis
        weight_calculator.print_weight_analysis(
            self.class_weights,
            class_names,
            class_counts_dict,  # Pass dict instead of list
        )

        # Mixed precision scaler
        self.scaler = torch.amp.GradScaler("cuda")

        # 3. Disable mixed precision temporarily for diagnosis
        self.use_amp = False  # Add this line - was missing!
        print("🔍 Running in FULL PRECISION mode for maximum stability")
    ...
    def train_epoch(self):
        """Ultra-stable training epoch"""
        self.model.train()
        total_loss = 0
        correct = 0
        total = 0
        valid_batches = 0
        nan_streak = 0  # Track consecutive NaN batches
        max_nan_streak = 10  # Stop if we hit too many consecutive NaNs
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # EMERGENCY: Check for NaN cascade
            if not torch.isfinite(loss):
                nan_streak += 1
                print(f"⚠️ NaN loss in batch {batch_idx} (streak: {nan_streak})")

                if nan_streak >= max_nan_streak:
                    print(f"🛑 STOPPING: {nan_streak} consecutive NaN losses detected!")
                    print("💡 Model has entered unstable regime - training terminated")
                    return float("inf"), 0.0

                continue  # Skip this batch entirely
            else:
                nan_streak = 0  # Reset streak on successful batch
            ...
            if self.use_amp:
                ...
                if not torch.isfinite(grad_norm):
                    print(f"⚠️ NaN gradients in batch {batch_idx}, skipping...")
                    continue

                self.optimizer.step()

            # At the end, for ReduceLROnPlateau only:
            # if isinstance(self.scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
            #     # Don't step here - step in main training loop with validation loss
            #     pass
            # else:
            #     self.scheduler.step()
        ...
        return total_loss / valid_batches, 100.0 * correct / total
...
def run_multimodal_training():
    ...
    try:
        ...
        # Fix: Use the correct method for StratifiedVideoDatasetSplitter
        ...
        # Initialize results saver - Fix the parameters
        experiment_name = (
            f"multimodal_2stream_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )

        # Use correct ResultsSaver initialization
        ...
        # Initialize trainer first to get class weights
        ...
        # Now save experiment config with class weights info
        try:
            class_weights_info = {
                "weighting_strategy": "inverse_sqrt",
                "rare_class_boost": 2.0,
                "class_weights": trainer.class_weights.cpu().tolist(),
                "class_names": class_names,
                "class_counts": class_counts_array,
            }
            results_saver.save_experiment_config(
                config, model_params, class_weights_info
            )
        ...
        try:
            create_class_weights_plots(
                ...
                class_names=class_names,
                class_counts=class_counts_array,
                save_dir=results_saver.experiment_dir / "plots",
            )
        ...
        # Create training curves - Fix the function call
        try:
            create_training_curves(
                training_history=results_saver.training_history,  # Use results_saver's training_history
                save_dir=results_saver.experiment_dir / "plots",
            )
