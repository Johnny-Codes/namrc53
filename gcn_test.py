...
NUM_FOLDERS = 34
SEQUENCE_LENGTH = 150
BATCH_SIZE = 16  # GCNs can be memory intensive, so a smaller batch size is safer
NUM_EPOCHS = 50  # Increased for a more complex model
LEARNING_RATE = 0.0005  # Slightly lowered for more stable training with a scheduler
MODEL_SAVE_PATH_INNER = "best_inner_camera_gcn_model.pth"
MODEL_SAVE_PATH_OUTER = "best_outer_camera_gcn_model.pth"
...
def get_adjacency_matrix():
    ...
    D_hat_inv_sqrt = np.diag(D_hat_inv_sqrt)
    return torch.tensor(D_hat_inv_sqrt @ A_hat @ D_hat_inv_sqrt, dtype=torch.float32)
...
class STGCNBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dropout=0.5):
        ...
        # --- Residual Connection ---
        # If the number of channels changes, we need a 1x1 convolution to match dimensions.
    ...
    def forward(self, x, adj):
        res = self.residual(x.permute(0, 3, 1, 2)).permute(
            0, 2, 3, 1
        )  # Apply residual to original x
        x = self.gcn(x, adj)
...
# --- 3. Custom PyTorch Dataset for GCN ---
class GCN_SkeletonActionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sequence_np = np.array(sequence).reshape(-1, NUM_JOINTS, NUM_COORDS)
        pelvis_coords = sequence_np[:, 0:1, :]
        normalized_sequence = sequence_np - pelvis_coords
...
def train_and_evaluate_model(
    ...
    print(f"\n--- Starting Training for {model_name} GCN Model ---")
    best_accuracy = 0.0
    ...
    print(f"\n--- Final Testing for {model_name} GCN Model ---")
...
if __name__ == "__main__":
    ...
    for view_name, sequences_dict, save_path in [
        ...
        train_dataset = GCN_SkeletonActionDataset(train_data, train_labels)
        val_dataset = GCN_SkeletonActionDataset(val_data, val_labels)
        test_dataset = GCN_SkeletonActionDataset(test_data, test_labels)
